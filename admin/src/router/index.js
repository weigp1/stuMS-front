...
// TODO: 记得取消注释
  ...
  try {
    ...
    // sid 不存在, 则调用接口根据 token 获取用户信息
    if (!userStore.sid) {
      // await userStore.getUserInfo()
      const localInfo = getLocal('loginInfo')
      // console.log(localInfo.SID)
      await userStore.getUserInfo(localInfo.SID)
    }
    ...
    const accessRoutes = permissionStore.generateRoutesFront(['admin']) // ! 前端生成路由 (根据角色), 待完善
    // console.log(accessRoutes)
