...
function createPermissionGuard(router) {
  ...
  router.beforeEach(async (to) => {
    ...
    if (!token) {
      ...
      // if (['/login', '/404','/student/list','/user/list'].includes(to.path)) {
    ...
    // 如果用户角色为1且尝试访问 /user 路由，重定向到首页或其他页面
    if (role === 1 && to.path === '/user/list') {
      window.$message.error('无权访问该页面')
      return { path: '/home' } // 重定向到首页或者其他页面
    }
